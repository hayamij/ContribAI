...
impl Default for Stats {
    fn default() -> Self {
        Stats { total: 0, merged: 0, closed: 0, open: 0, repos_analyzed: 0 }
    }
}
...
impl App {
    fn new(config: &ContribAIConfig) -> Self {
        ...
        if !prs.is_empty() { pr_state.select(Some(0)); }
        let mut repo_state = ListState::default();
        if !repos.is_empty() { repo_state.select(Some(0)); }
    ...
    fn move_down(&mut self) {
        match self.tab {
            ...
            Tab::Actions => {
                ...
                self.action_state.select(Some((i + 1).min(ACTIONS.len() - 1)));
            }
            _ => {}
        }
    }
...
fn load_data(config: &ContribAIConfig) -> anyhow::Result<(Vec<PrRow>, Vec<String>, Stats)> {
    ...
    let prs: Vec<PrRow> = pr_maps.into_iter().map(|m| PrRow {
    ...
    let mut repos: Vec<String> = prs.iter()
        .map(|p| p.repo.clone())
    ...
    let stats = Stats { total, merged, closed, open: total - merged - closed, repos_analyzed: repos_a };
    Ok((prs, repos, stats))
}
...
fn render_title(f: &mut Frame, area: Rect) {
    let title = Paragraph::new(Line::from(vec![
        Span::styled(" 🤖 ContribAI ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled("v5.1.0 — Interactive Mode  ", Style::default().fg(Color::DarkGray)),
        Span::styled("[Tab] Switch  [1-4] Jump  [?] Help  [q] Quit", Style::default().fg(Color::DarkGray)),
    ]));
...
fn render_dashboard(f: &mut Frame, area: Rect, app: &App) {
    ...
    let merge_rate = if app.stats.total > 0 { app.stats.merged * 100 / app.stats.total } else { 0 };

    let stats_text = vec![
        Line::from(""),
        Line::from(vec![Span::raw("  📊 Total PRs:      "),
            Span::styled(app.stats.total.to_string(), Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))]),
        Line::from(vec![Span::raw("  ✅ Merged:         "),
            Span::styled(app.stats.merged.to_string(), Style::default().fg(Color::Green))]),
        Line::from(vec![Span::raw("  ❌ Closed:         "),
            Span::styled(app.stats.closed.to_string(), Style::default().fg(Color::Red))]),
        Line::from(vec![Span::raw("  🟡 Open:           "),
            Span::styled(app.stats.open.to_string(), Style::default().fg(Color::Yellow))]),
        Line::from(vec![Span::raw("  🎯 Merge rate:     "),
            Span::styled(format!("{}%", merge_rate), Style::default().fg(
                if merge_rate >= 70 { Color::Green }
                else if merge_rate >= 40 { Color::Yellow }
                else { Color::Red }
            ))]),
        Line::from(vec![Span::raw("  🔬 Repos analyzed: "),
            Span::styled(app.stats.repos_analyzed.to_string(), Style::default().fg(Color::Cyan))]),
        Line::from(""),
    ];
    ...
    let hints = vec![
        Line::from(""),
        Line::from(vec![Span::styled("  Quick keys:", Style::default().add_modifier(Modifier::BOLD))]),
        Line::from(""),
        Line::from(vec![Span::styled("  [2] ", Style::default().fg(Color::Yellow)), Span::raw("Browse PRs")]),
        Line::from(vec![Span::styled("  [3] ", Style::default().fg(Color::Yellow)), Span::raw("Browse Repos")]),
        Line::from(vec![Span::styled("  [4] ", Style::default().fg(Color::Yellow)), Span::raw("Actions")]),
        Line::from(vec![Span::styled("  [?] ", Style::default().fg(Color::Yellow)), Span::raw("Help")]),
        Line::from(vec![Span::styled("  [q] ", Style::default().fg(Color::Yellow)), Span::raw("Quit")]),
    ];

    f.render_widget(
        Paragraph::new(hints)
            .block(Block::default().title(" 💡 Navigation ")
                .borders(Borders::ALL).border_type(BorderType::Rounded)
    ...
    let rows: Vec<Row> = app.prs.iter().take(25).map(|pr| {
        let (style, icon) = pr_style_icon(&pr.status);
        Row::new(vec![
            Cell::from(pr.number.as_str()).style(Style::default().fg(Color::Cyan)),
            Cell::from(pr.repo.chars().take(20).collect::<String>()).style(Style::default().fg(Color::DarkGray)),
    ...
    let table = Table::new(
        rows,
        [Constraint::Length(5), Constraint::Length(20), Constraint::Min(0), Constraint::Length(12)],
    )
    .header(Row::new(vec!["#", "Repo", "Title", "Status"])
        .style(Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))
        .bottom_margin(1))
    .block(Block::default().title(format!(" 📋 Recent PRs ({}) ", app.prs.len()))
        .borders(Borders::ALL).border_type(BorderType::Rounded)
        .border_style(Style::default().fg(Color::Cyan)));
...
fn render_prs(f: &mut Frame, area: Rect, app: &App) {
    let rows: Vec<Row> = app.prs.iter().map(|pr| {
        let (style, icon) = pr_style_icon(&pr.status);
        Row::new(vec![
            Cell::from(pr.number.as_str()).style(Style::default().fg(Color::Cyan)),
            Cell::from(pr.repo.chars().take(28).collect::<String>()).style(Style::default().fg(Color::DarkGray)),
    ...
    let table = Table::new(
        rows,
        [Constraint::Length(5), Constraint::Length(28), Constraint::Min(0), Constraint::Length(12), Constraint::Length(16)],
    )
    .header(Row::new(vec!["#", "Repository", "Title", "Status", "Type"])
        .style(Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))
        .bottom_margin(1))
    .highlight_style(Style::default().bg(Color::DarkGray).add_modifier(Modifier::BOLD))
    .block(Block::default().title(format!(" 📋 Pull Requests ({}) ", app.prs.len()))
        .borders(Borders::ALL).border_type(BorderType::Rounded)
        .border_style(Style::default().fg(Color::Cyan)));
...
fn render_repos(f: &mut Frame, area: Rect, app: &App) {
    let items: Vec<ListItem> = app.repos.iter().map(|r| {
        let total  = app.prs.iter().filter(|p| &p.repo == r).count();
        let merged = app.prs.iter().filter(|p| &p.repo == r && p.status == "merged").count();
        let rate = if total > 0 { merged * 100 / total } else { 0 };
        ListItem::new(Line::from(vec![
            Span::styled(format!("  {:<35}", r.chars().take(34).collect::<String>()), Style::default().fg(Color::Cyan)),
            Span::styled(format!(" {:>3} PRs", total), Style::default().fg(Color::DarkGray)),
            Span::styled(format!("  {:>3} merged", merged), Style::default().fg(Color::Green)),
            Span::styled(format!("  {}%", rate), Style::default().fg(
                if rate >= 70 { Color::Green } else if rate >= 40 { Color::Yellow } else { Color::Red }
            )),
        ]))
    }).collect();
    ...
    let list = List::new(items)
        .highlight_style(Style::default().bg(Color::DarkGray).add_modifier(Modifier::BOLD))
        .block(Block::default().title(format!(" 🔬 Repositories ({}) ", app.repos.len()))
            .borders(Borders::ALL).border_type(BorderType::Rounded)
            .border_style(Style::default().fg(Color::Cyan)));
...
fn render_actions(f: &mut Frame, area: Rect, app: &App) {
    ...
    let items: Vec<ListItem> = ACTIONS.iter().map(|(icon, name, _)| {
        ListItem::new(Line::from(vec![
            Span::raw(format!("  {} ", icon)),
            Span::styled(format!("{:<22}", name), Style::default().add_modifier(Modifier::BOLD)),
        ]))
    }).collect();
    ...
    let list = List::new(items)
        ...
        .block(Block::default().title(" ⚡ Actions (j/k to select) ")
            .borders(Borders::ALL).border_type(BorderType::Rounded)
            .border_style(Style::default().fg(Color::Cyan)));

    f.render_stateful_widget(list, layout[0], &mut state);

    let idx = app.action_state.selected().unwrap_or(0).min(ACTIONS.len() - 1);
    let (icon, name, cmd) = ACTIONS[idx];

    let detail = vec![
        Line::from(""),
        Line::from(vec![
            Span::raw("  "), Span::styled(icon, Style::default().fg(Color::Cyan)),
            Span::styled(format!(" {}", name), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
        Line::from(vec![Span::styled("  CLI Command:", Style::default().fg(Color::DarkGray))]),
        Line::from(""),
        Line::from(vec![Span::raw("  "), Span::styled(cmd, Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))]),
        Line::from(""),
        Line::from(vec![Span::styled("  Copy & run in your terminal", Style::default().fg(Color::DarkGray))]),
    ];

    f.render_widget(
        Paragraph::new(detail).wrap(Wrap { trim: true })
            .block(Block::default().title(" 📋 Selected Command ")
                .borders(Borders::ALL).border_type(BorderType::Rounded)
                .border_style(Style::default().fg(Color::DarkGray))),
        layout[1],
    );
...
fn render_help(f: &mut Frame, area: Rect) {
    ...
    let text = vec![
        ...
        Line::from(""),
        Line::from(vec![Span::styled("  Press any key to close", Style::default().fg(Color::DarkGray).add_modifier(Modifier::ITALIC))]),
        Line::from(""),
    ];
